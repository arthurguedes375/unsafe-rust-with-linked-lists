...
impl<T> Node<T> {
    ...
    pub fn insert_in_place(&mut self, val: T) {
         ...
         * Instead of all of this unsafe code you could just do: 'self.next.take()' (which would
        ...
        let next_val = unsafe {
             ...
             * Since the self.next is just a Option with a box pointer it is not expansive.
             */
        ...
        let new_node = Box::new(Node {
            value: val,
            // next: self.next.take(), // Instead of using all of the code above we can just call self.next.take() and it will work just fine, but since this is a learning project there you go...
            next: next_val,
        });
